...
@Component({
    ...
    @if (testimonials().length > 0) {
      <section class="testimonials">
        <div class="container">
          ...
          <h2>Loved by Ladies</h2>
          <div class="testimonials-grid">
            @for (t of testimonials(); track t.id) {
              <div class="t-card">
                @if (t.hasImage) {
                  <img class="t-img" [src]="testimonialSvc.imageUrl(t.id)" alt="Customer photo" />
                } @else {
  ...
  styles: [`
    ...
    /* ── Testimonials ─────────────────────────────────────────────────────── */
    .testimonials { padding: 4rem 0; background: #fafafa; }
    .section-tag {
      text-align: center; font-size: 0.75rem; font-weight: 700;
      letter-spacing: 0.14em; text-transform: uppercase;
      color: #e8468c; margin: 0 0 0.6rem;
    }
    .testimonials h2 {
      font-family: 'Playfair Display', serif; font-size: clamp(1.5rem, 3vw, 2.2rem);
      text-align: center; margin: 0 0 2.5rem; color: #2d2d2d;
    }
    .testimonials-grid {
      display: grid;
    ...
    @media (max-width: 480px) {
      .testimonials-grid { grid-template-columns: repeat(2, 1fr); gap: 0.75rem; }
    }
  `]
})
